...
if !!node['blueplains']['db_setup'] == true
	...
	database 'mahlab_blueplains' do
	  connection conn_info
		provider Chef::Provider::Database::Mysql
	  sql { ::File.open("#{node['blueplains']['src_root']}/sql/db-model.sql").read }
	  action :query
	end
	  ...
	  connection conn_info
		provider Chef::Provider::Database::Mysql
	  sql { ::File.open("#{node['blueplains']['src_root']}/sql/db-data.sql").read }
	  action :query
	end
...
web_app "insuranceandrisk" do
	...
	docroot "#{node['blueplains']['src_root']}/#{node['blueplains']['irp_docroot']}"
...
web_app "probus" do
	...
	server_aliases [ node['fqdn'] ]
	docroot "#{node['blueplains']['src_root']}/#{node['blueplains']['pb_docroot']}"
...
web_app "blueplains-cms" do
	...
	server_aliases [ node['fqdn'] ]
	docroot "#{node['blueplains']['src_root']}/#{node['blueplains']['cms_docroot']}"
...
web_app "blueplains-microsite" do
	...
	server_aliases [ node['fqdn'] ]
	docroot "#{node['blueplains']['src_root']}/#{node['blueplains']['microsite_docroot']}"
...
template "#{node['blueplains']['src_root']}/blueplains/app-public/config/core.php" do
  source "blueplains/app-public/config/core.php.erb"
  mode 00644
end
template "#{node['blueplains']['src_root']}/blueplains/app-private/config/core.php" do
  source "blueplains/app-private/config/core.php.erb"
  mode 00644
end
template "#{node['blueplains']['src_root']}/blueplains/app-microsite/config/core.php" do
